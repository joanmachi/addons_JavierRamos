...
class LiraAgingSupplierLine(models.Model):
    _name = 'lira.aging.supplier.line'
    _description = 'Línea antigüedad de saldos proveedores'
    _order = 'total desc'
    ...
    partner_ref  = fields.Char(related='partner_id.ref', string='Ref. proveedor', store=False)
    ...
    total        = fields.Float('Total a pagar (€)', digits=(16, 2))
    num_facturas = fields.Integer('Facturas')
    dias_max     = fields.Integer('Días máx. vencido')

    def action_open_source(self):
        """Abre las facturas pendientes de pago del proveedor en Odoo."""
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': f'Facturas de proveedor pendientes — {self.partner_id.name}',
...
class LiraAgingSupplier(models.TransientModel):
    _name = 'lira.aging.supplier'
    _description = 'Antigüedad de saldos proveedores (a pagar)'
    _rec_name = 'display_title'

    display_title     = fields.Char(default='Antigüedad de Saldos — Proveedores', readonly=True)
    total_pendiente   = fields.Float('Total a pagar (€)', readonly=True)
    ...
    proveedores_vencidos = fields.Integer('Proveedores con deuda vencida', readonly=True)
    ...
    def _build_data(self):
        ...
        for pid, g in groups.items():
            if not g['partner']:
                continue
            total = g['corriente'] + g['tramo_30'] + g['tramo_60'] + g['tramo_90'] + g['tramo_mas']
            lines_data.append({
                'partner_id':   pid,
        ...
        kpis = {
            ...
            'proveedores_vencidos': sum(1 for d in lines_data if (d['tramo_30'] + d['tramo_60'] + d['tramo_90'] + d['tramo_mas']) > 0),
        }
    ...
    def _compute_and_store(self):
        for rec in self:
            ...
            Line = self.env['lira.aging.supplier.line']
    ...
    def action_ver_tabla(self):
        ...
        lv = self.env.ref('lira_dashboard_contabilidad.view_lira_aging_supplier_line_list', raise_if_not_found=False)
        sv = self.env.ref('lira_dashboard_contabilidad.view_lira_aging_supplier_line_search', raise_if_not_found=False)
        action = {
            'type':      'ir.actions.act_window',
            'name':      'Antigüedad proveedores — detalle',
            'res_model': 'lira.aging.supplier.line',
            'view_mode': 'list',
    ...
    def action_open(self):
        ...
        return {
            'type': 'ir.actions.act_window', 'name': 'Antigüedad de Saldos — Proveedores',
