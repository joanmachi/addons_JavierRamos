...
class L10nEsAeatmod592LineMixin(models.AbstractModel):
    ...
    def _compute_kgs(self):
        for item in self:
            if item.stock_move_id.picking_type_id.code == 'incoming':
                item.kgs = (item.product_id.plastic_tax_weight * -1) * item.product_uom_qty
            else:
        ...
        for item in self:
            if item.stock_move_id.picking_type_id.code == 'incoming':
                item.no_recycling_kgs = (
                    (item.product_id.plastic_weight_non_recyclable * -1) * item.product_uom_qty
                )
            else:
                item.no_recycling_kgs = (
