...
class ApuntsWipValuationLine(models.Model):
    _name = 'apunts.wip.valuation.line'
...
class ApuntsWipValuation(models.TransientModel):
    _name = 'apunts.wip.valuation'
    ...
    def _apunts_get_product_cost(self, product, production=None):
        """Cascada con prioridad a PO vinculada a la OF (campo fabricacion JR
        + procurement_group + origin). Soporta UoM secundaria (price_subtotal/qty).
        """
        if not product:
            return 0.0
        POL = self.env['purchase.order.line']
        # 0a. Vínculo manual JR via campo `fabricacion`
    ...
    def _calc_value(self, sol, prod, production_state, mp_state, delivery_state, invoice_state):
        ...
        if prod:
            if production_state in ('progress', 'to_close'):
                # Regla 4: MP consumida + M.O. acumulada
                coste_mp = 0.0
                for m in prod.move_raw_ids:
                    ...
                    if not price_unit:
                        price_unit = self._apunts_get_product_cost(m.product_id, production=prod)
            ...
            if production_state == 'confirmed':
                # Regla 3: MP reservada → coste MP previsto
                ...
                # Reglas 1 y 2: sin MP o MP pedida sin recibir → 0
                return 0.0, 0.0, 0.0
    ...
    def _compute_and_store(self):
        for rec in self:
            ...
            Line = self.env['apunts.wip.valuation.line']
    ...
    def action_ver_tabla(self):
        ...
        lv = self.env.ref('apunts_wip.view_apunts_wip_valuation_line_list', raise_if_not_found=False)
        sv = self.env.ref('apunts_wip.view_apunts_wip_valuation_line_search', raise_if_not_found=False)
        action = {
            'type': 'ir.actions.act_window',
            'name': 'Valoración de producto en curso — detalle',
            'res_model': 'apunts.wip.valuation.line',
            'view_mode': 'list',
